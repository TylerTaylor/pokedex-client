...
export const fetchCards = (query, pageNum, sortFilter = null, filters = null) => {
    ...
    let cardsAPI = 'http://mondex-api.herokuapp.com/api/v1/cards';

    // the link NEEDS to be like:
    // http://mondex-api.herokuapp.com/api/v1/cards?firstParam&subsequentParams
    // ? first then &
    ...
    if (!query && !pageNum && !sortFilter && !filters) {
        cardsAPI = 'http://mondex-api.herokuapp.com/api/v1/cards'
    }
...
export const fetchCard = (cardID, token) => {
    return dispatch => {
        return fetch(`http://mondex-api.herokuapp.com/api/v1/cards/${cardID}`, {
            headers: {
...
export const fetchCollection = (token, pageNum) => {
    ...
    return dispatch => {
        return fetch(`http://mondex-api.herokuapp.com/api/v1/collection?page=${pageNum}`, {
            headers: {
                "Accept":"application/json",
                "Content-Type":"application/json",
                "Authorization":`Bearer: ${token}`
...
export const addToCollection = (cardID, token) => {
    return dispatch => {
        return fetch(`http://mondex-api.herokuapp.com/api/v1/cards/${cardID}/add`, {
            method: 'post',
            headers: {
                "Accept":"application/json",
                "Content-Type":"application/json",
